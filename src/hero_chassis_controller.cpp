...
//通过effort_joint_interface->getHandle("...")这样的用法，对传入的effort_joint_interface对象指针进行操作，调用了getHandle函数并传入了关节名称作为参数。
    bool HeroChassisController::init(hardware_interface::EffortJointInterface *effort_joint_interface, ros::NodeHandle &root_nh , ros::NodeHandle &n)
    {//这是用来从EffortJointInterface对象中获取特定名称关节的控制句柄，然后将这些控制句柄赋值给了类成员变量front_left_joint_、front_right_joint_、back_left_joint_、back_right_joint_。
        ...
        //创建一个/cmd_vel的订阅器
        cmd_vel_sub_ = n.subscribe<geometry_msgs::Twist>("/cmd_vel", 1, &HeroChassisController::setCommandCB, this);
        ...
        void JointVelocityController::setCommandCB(geometry_msgs::Twist cmd)
        {
            cmd_vel_ = cmd;//该函数将传入的Twist对象cmd的值赋给了cmd_vel_这个类中的变量
        }
        // Return the current velocity command
        void JointVelocityController::getCommandCB(geomtry_msgs::Twist& cmd)
        {
            cmd = cmd_vel_;//这个函数通过参数的引用返回了控制器的命令值。
        }
        ...
        {
            now = time;
            vel_now[1] = front_left_joint_.getVelocity();//实际值
            vel_now[2] = front_right_joint_.getVelocity();
            vel_now[3] = back_left_joint_.getVelocity();
            vel_now[4] = back_right_joint_.getVelocity();
            ...
            for (int i = 1; i < 5; i++)
            {
             ...
             //pid控制器计算差值并进行调整
             double commanded_effort_front_left = pid_front_left_.computeCommand(error[1],period);
             double commanded_effort_front_right = pid_front_right_.computeCommand(error[2],period);
             double commanded_effort_back_left = pid_back_left_.computeCommand(error[3],period);
             double commanded_effort_back_right = pid_back_right_.computeCommand(error[4],period);
            ...
            if(loop_count_ % 10 == 0)
             {
                 ...
                 controller_state_publisher_->msg_.set_point = cmd_vel[i];
                 controller_state_publisher_->msg_.process_value = vel_now[i];
                 controller_state_publisher_->msg_.error = error[i];
    ...
    void HeroChassisController::compute_the_cmd()
    {
        ...
        vel_cmd[4] = (lx - ly + l * az) * wheel_radius_;
    }

    //计算每个车轮的PID控制(控制器根据PID传入的参数进行调节)
    double front_left_effort = pid_front_left_.computeCommand(vel_cmd[1],period);
    double front_right_effort = pid_front_right_.computeCommand(vel_cmd[2],period);
    double back_left_effort = pid_back_left_.computeCommand(vel_cmd[3],period);
    double back_right_effort = pid_back_right_.computeCommand(vel_cmd[4],period);
    ...
    void HeroChassisController::compute_the_cur_vel()
    {//根据论文中的正运动学计算公式
        vx = (vel_now[1] + vel_now[2] + vel_now[3] + vel_now[4]) * wheel_radius_ / 4;
        vy = (-vel_now[1] + vel_now[2] + vel_now[3] - vel_now[4]) * wheel_radius_ / 4;
        vth = (-vel_now[1] + vel_now[2] - vel_now[3] + vel_now[4]) * wheel_radius_/4l;
    }
