...
//通过effort_joint_interface->getHandle("...")这样的用法，对传入的effort_joint_interface对象指针进行操作，调用了getHandle函数并传入了关节名称作为参数。
    ...
    {//这是用来从EffortJointInterface对象中获取特定名称关节的控制句柄，然后将这些控制句柄赋值给了类成员变量front_left_joint_、front_right_joint_、back_left_joint_、back_right_joint_。
        ...
        //创建一个/cmd_vel的订阅器
        cmd_vel_sub_ = controller_nh.subscribe<geometry_msgs::Twist>("/cmd_vel", 1, &HeroChassisController::setCommandCB, this);
        ...
        void HeroChassisController::update(const ros::Time& time, const ros::Duration& period) {
            ...
            if (loop_count_ % 10 == 0) {
                if (controller_state_publisher_ && controller_state_publisher_->trylock()) {
                    ...
                    controller_state_publisher_->msg_.error = error[4];
            ...
            void HeroChassisController::setCommandCB(
                    const geometry_msgs::Twist::ConstPtr &msg) {   //逆运动学是根据机器人的位姿来计算对应部分的运行状态的，这个机器人运动总结来说就是三种（x方向的位移，y方向的位移和绕z轴的旋转），所以就用到这三个数据
    ...
    void HeroChassisController::setCommandCB(const int &msg)
    {
